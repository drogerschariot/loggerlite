...
task :deploy do
	sh "ssh -p #{MASTER_PORT} #{USER}@#{HOST} 'rm -rf /etc/puppet/modules/#{MODULE}'"
	sh "#{SCP} -P #{MASTER_PORT} -i ~/.ssh/id_puppettest ~/projects/puppet/#{MODULE} #{USER}@#{HOST}:/etc/puppet/modules/#{MODULE}"
